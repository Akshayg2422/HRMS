...
import { Sort, CommonTable, Modal, Carousel , Table, NoRecordFound} from '@components'
...
function PortFolio() {
    ...
    const { employeeCheckInLogs, employeeCheckInDetailedLogPerDay, employeeEachUserSheets } = useSelector(
    ...
    const normalizedPerDayData = (data: any) => {
        return data.map((it: any) => {
            return {
                ...
                Time: getDisplayTimeFromMoment(
                    getMomentObjFromServer(it.checkin_time),
                ),
                Type: it.type,
    ...
    return (
        <>
            <div className='row'>
                <div className='col-xl-7 col-md-12 mt-3'>
                    <div className='col  text-right mb-3'>
                        <Sort sortData={sortData} activeIndex={activeSortWorkBook} onClick={(index) => {
            ...
            <Modal showModel={logPerDayModel} toggle={() => setLogPerDayModel(!logPerDayModel)}>
                {
                    employeeCheckInDetailedLogPerDay && employeeCheckInDetailedLogPerDay.length > 0 ? <Table tableDataSet={normalizedPerDayData(employeeCheckInDetailedLogPerDay)} /> : <NoRecordFound />
                }
            </Modal>
        </>
    )
