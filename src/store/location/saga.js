...
import { fetchAllBranchesList,postBranchAddition, updateBranchLocationRadius,postEnableBranchRefence,fetchEmployeeCheckinAssociations,
  postEmployeeCheckinAssociations} from "../../helpers/backend_helper";
import { FETCH_ALL_BRANCHES_LIST,POST_BRANCH_ADDITION,UPDATE_BRANCH_LOCATION_RADIUS,ENABLE_BRANCH_REFENCE,
GET_EMPLOYEE_CHECKIN_ASSOCIATIONS,
UPDATE_EMPLOYEE_CHECKIN_ASSOCIATIONS} from "./actionsType";
import { getAllBranchesListFailure,getAllBranchesListSuccess,branchAdditionSuccess,branchAdditionFailure,
  ...
  updatetEmployeeCheckinAssociationsFailure} from "./actions";
...
function* getAllBranches(action) {
  ...
  function* getEmployeeCheckinAssociations(action) {
    try {
      const response = yield call(fetchEmployeeCheckinAssociations, action.payload.params);
      if (response.success) {
